...
public class Game extends Application implements GameScreen {
        ...
        for(int y = 0; y < this.height; ++y) {
            for(int x = 0; x < this.width; ++x) {
                ...
                if (this.showGrid && children.size() > 0) {
                    ...
                    cell.setStroke(this.toFXColor(engine.cell.Color.BLACK));
                }
    ...
    private void createBorderImage() {
        InputStream inputStream = Game.class.getResourceAsStream("/resources/myscreen.png");
    ...
    private void createScorePanel() {
        ...
        this.scoreText.setFill(this.toFXColor(engine.cell.Color.BLACK));
        StackPane scorePane = new StackPane(new Node[]{this.scoreText});
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(engine.cell.Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(engine.cell.Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(engine.cell.Color.WHITE));
    ...
    public void setCellColor(int x, int y, engine.cell.Color color) {
        if (color != null && color != engine.cell.Color.NONE) {
            ObservableList<Node> children = this.cells[y][x].getChildren();
    ...
    public void setCellTextColor(int x, int y, engine.cell.Color color) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    public engine.cell.Color getCellTextColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    private engine.cell.Color fromFXColor(Color color) {
        ...
        engine.cell.Color[] result = new engine.cell.Color[]{engine.cell.Color.NONE};
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = engine.cell.Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }

    private Color toFXColor(engine.cell.Color color) {
        if (color == engine.cell.Color.NONE) {
    ...
    public void setCellValueEx(int x, int y, engine.cell.Color cellColor, String value) {
        this.setCellValue(x, y, value);
        this.setCellColor(x, y, cellColor);
    }

    public void setCellValueEx(int x, int y, engine.cell.Color cellColor, String value, engine.cell.Color textColor) {
        this.setCellValueEx(x, y, cellColor, value);
        this.setCellTextColor(x, y, textColor);
    }

    public void setCellValueEx(int x, int y, engine.cell.Color cellColor, String value, engine.cell.Color textColor, int textSize) {
        this.setCellValueEx(x, y, cellColor, value, textColor);
        this.setCellTextSize(x, y, textSize);
    }

    public void showMessageDialog(engine.cell.Color cellColor, String message, engine.cell.Color textColor, int textSize) {
    ...
    public void setLives(int lives) {
        if (this.livesText == null) {
            ...
            this.livesText.setFill(this.toFXColor(engine.cell.Color.ORANGE));
